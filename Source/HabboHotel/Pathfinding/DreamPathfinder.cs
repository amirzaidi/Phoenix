...
namespace Phoenix.HabboHotel.Pathfinding
{
	internal sealed class DreamPathfinder
	{
		...
		private static SquarePoint GetClosetSqare(SquareInformation pInfo, HeightInfo Height, bool bool_0)
		{
        ...
        internal static SquarePoint GetNextStep(int pUserX, int pUserY, int pUserTargetX, int pUserTargetY, byte[,] pGameMap, double[,] pHeight, double[,] double_1, double[,] double_2, int MaxX, int MaxY, bool pUserOverride, bool pDiagonal)
        {
            ...
            return DreamPathfinder.GetClosetSqare(pInfo, new HeightInfo(MaxX, MaxY, pHeight, double_1, double_2), pDiagonal);
        }
	}
}
