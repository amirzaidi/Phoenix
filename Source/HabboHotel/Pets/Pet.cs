	...
	internal sealed class Pet
	{
		...
		public Room Room
		{
			get
			{
				if (!IsInRoom)
				{
					...
					return Phoenix.GetGame().GetRoomManager().GetRoom(RoomId);
			}
		}
		...
		public int ExpirienceGoal
		{
			get
			{
				...
				{
					level = this.experienceLevels[this.Level - 1];
				}
				else
				{
					level = 240000;
		...
		public int Age
		{
			get
			{
				return (int)Math.Floor((Phoenix.GetUnixTimestamp() - this.CreationStamp) / 86400.0);
			}
		}
		public string Look
		{
		...
		public string OwnerName
		{
			get
			{
				return Phoenix.GetGame().GetClientManager().GetNameById(OwnerId);
			}
		}
