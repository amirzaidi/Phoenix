...
namespace Phoenix.HabboHotel.Quests
{
	internal sealed class Quest
	{
		...
		public void Serialize(ServerMessage Message, GameClient Session, bool Single)
		{
			...
			Message.AppendInt32(Phoenix.GetGame().GetQuestManager().GetHighestLevelForType(this.Type));
			if (Phoenix.GetGame().GetQuestManager().GetHighestLevelForType(this.Type) == this.Level && Session.GetHabbo().CompletedQuests.Contains(this.Id) && !Single)
			{
